# GrapheneUX
Forthcoming hack instructions (star/follow the repo so you don't miss the update!)
- [x] [Reinstate pattern unlock!](https://github.com/e-t-l/GrapheneUX/blob/main/Pattern%20lock.md)
- [x] [Shizuku system privileges! (non-root)](https://github.com/e-t-l/GrapheneUX/blob/main/Shizuku%20system%20privileges.md)
- [x] [On-device ADB shell](https://github.com/e-t-l/GrapheneUX/blob/main/On-device%20ADB.md)
- [X] [Replace Graphene Camera with Google Camera](https://github.com/e-t-l/GrapheneUX/blob/main/Google%20Pixel%20Camera%20override.md)
- [X] [Double-tap back of phone (reverse-engineered)](https://github.com/e-t-l/GrapheneUX/blob/main/Tap%20Phone%20Back.md)
- [x] [Reinstate *Now Playing*](https://github.com/e-t-l/GrapheneUX/blob/main/Now%20Playing.md)
- [ ] Reinstate *Call Screening* (WIP)
- [ ] Reinstate *Hold For Me*
- [X] [Copy/OCR text from screen or image](https://github.com/e-t-l/GrapheneUX/blob/main/Copy%20text%20%2B%20OCR.md)
- [X] [Disable the Work Profile's lock PIN](https://github.com/e-t-l/GrapheneUX/blob/main/Lockless%20work.md)
- [ ] Voice typing
- [X] [Full-brightness flashlight (brighter than on stock Pixel OS!)](https://github.com/e-t-l/GrapheneUX/blob/main/Brighter%20flashlight.md)
- [ ] Quick Settings tiles responsive while locked (reverse-engineered)
- [ ] Weather widget on lockscreen
- [ ] ANY WIDGET on lockscreen!
- [ ] BONUS: Rooted GrapheneOS w/ locked bootloader & Verified Boot (use at your own risk!)
